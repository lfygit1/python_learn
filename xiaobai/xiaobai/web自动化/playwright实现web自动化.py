...
with sync_playwright() as p:   # 通过上下文管理器启动playwright,确保资源能自动释放
    firefox=p.firefox.launch(headless=False,slow_mo=500)  # 启动chrome浏览器
    page = firefox.new_page()  # 创建一个新页面
    page.goto('https://www.bilibili.com/')   # 打开指定网址
    time.sleep(1)
    page.get_by_text('立即登录').click()
    page.get_by_placeholder('请输入账号').fill('18530571185')
    page.get_by_placeholder('请输入密码').fill('lfy,827241')
    ...
    # page.locator('#app > div > div > div:nth-child(2) > form > div:nth-child(5) > div > button').click()   # 用CSS定位登录按钮
    ...
    # page.get_by_role('emphasis',name="").click()   # 用 get_by_role 定位，点击上传按钮
